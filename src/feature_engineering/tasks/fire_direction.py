...
def calculate_cosine_similarity(x1, y1, x2, y2, direction_deg):
    ...
    vec_direction = np.array(
        [np.cos(np.radians(direction_deg)), np.sin(np.radians(direction_deg))]
    )

    # Calculate the cosine similarity
    cosine_similarity = np.dot(vec_to_point, vec_direction) / (
        np.linalg.norm(vec_to_point) * np.linalg.norm(vec_direction)
    )
...
def add_fire_direction_to_tensor(
    tensor: torch.Tensor, wind_direction_index: int, fire_mask_index: int
) -> torch.Tensor:
    ...
    for i in range(tensor.shape[0]):
        wind_influence = apply_cosine_similarity(
            fire_mask[i, :, :], wind_direction[i, :, :], calculate_cosine_similarity
        )
    ...
    tensor = torch.cat([tensor, fire_direction_final], dim=-1)
